...
while getopts "h?urd:v" opt; do
    case "$opt" in
	h|\?)
            echo "Usage:  renew_pdb_full.sh [-u|-r] (default: -u update only) [-v (verbose)] [-d dbName] (default: dbNew, if you give an existing directory name, the old content will be lost!) " 
            exit 0
            ;;
	u)  update=true
            ;;
...
then
    ...
    echo 'Removing old files! in ' $psipred_a3m_dir
...
if $verbose ;
then
    echo calling addss.pl via multithread on $psipred_a3m_dir \(see log \in $addss_log_dir\) 
...
# now move the new a3ms to the old directory (we couldn't do that before since addss.pl should only run on the new files)
if $update ;
then
    ...
    if $verbose ;
    then
	echo moved files from $a3m_dir to $a3m_dir_old 
    fi  
fi
