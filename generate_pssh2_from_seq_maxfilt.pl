...
then 
	/usr/bin/time /usr/bin/hhblits -i $i -d $uniprot20 -ohhm $ohhm -o $ohhr1 -maxfilt 100000
fi";
my $cmd_hhblits2 = "if [ ! -f $ohhr ] 
then 
	/usr/bin/time /usr/bin/hhblits -i $ohhm -d $pdb_full -n 1 -B $hit_list -Z $hit_list -o $ohhr -maxfilt 100000
fi";
