...
my $v=2;#
#Rostlab/benjamin edit: our blastpgp is just 'blastpgp'
#my $blastpgp="$ncbidir/blastpgp";
my $blastpgp="blastpgp";
my $help="
...
foreach $seq (@seqs) {
    ...
    open(BLAST,"$blastpgp -i $root.tmp -a 10 -d $root.db -v 1 -b 1000 -s T -z $ntot|");
    while ($line=<BLAST>){
