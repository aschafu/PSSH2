...
if ($fam) {
	...
	if ($line=~/^>(\S+)\s+(\S+)/) {
	    $famid=$2;
	    $n = printSeq($length, $countX, $sequence, $outfile, $n);
	...
	if ($line=~/^>(\S+)/) {
	    $n = printSeq($length, $countX, $sequence, $outfile, $n);
#	    print STDOUT join " ", ($length, $countX, $sequence, $outfile, $n, "\n");
	    if ($exists{$1}) {print STDERR ("\nWarning: id $1 appears more than once in $infile\n");}
	    $exists{$1}=1;
	    my $tmp = $1;
	    $tmp =~ s/\|/_/g;
	    $tmp =~ s/\./_/g;
...
sub printSeq {
    ...
    my ($length, $countX, $sequence, $outfile, $n) = @_;
 
    if ($length){
	my $ratio = $countX / $length;
	if ($ratio < $cutoff ){ # exclude sequence with more than half "X"
	    ...
	    print("The sequence in $outfile has $countX 'X' in sequence of length $length (ratio $ratio) - $outfile excluded.\n");
	}
    }
    return $n;
